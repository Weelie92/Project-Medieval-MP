    ...
    public class SplineModule
    {
        ...
        private float GetDistanceToMove( GameObject prefab)
        {
            ...
            switch ( prefabPainter.splineSettings.separation)
            {
                ...
                case SplineSettings.Separation.PrefabRightSize:
                    distanceToMove = GetPrefabRightSize(prefab);
                ...
                case SplineSettings.Separation.PrefabUpSize:
                    distanceToMove = GetPrefabUpSize(prefab);
        ...
        private float GetPrefabRightSize(GameObject prefab)
        {
            Bounds bounds = BoundsUtils.CalculateBounds(prefab);
        ...
        private float GetPrefabUpSize(GameObject prefab)
        {
            Bounds bounds = BoundsUtils.CalculateBounds(prefab);
        ...
        private void ApplyPrefabSettings( int offsetLane, PrefabSettings prefabSettings, GameObject prefab, Vector3 currentPosition, Vector3 direction, List<SplinePoint> splinePoints, int currentSplinePointIndex)
        {
            ...
            if (offsetLane == 0)
            {
                ...
                else if (prefabPainter.splineSettings.separation == SplineSettings.Separation.PrefabRightSize)
                {
                    // move along in the spline rotation, considering the extents (/2)
                    addDistanceToDirection = splineRotation * prefab.transform.right * GetDistanceToMove(prefab) / 2;
                }
                else if (prefabPainter.splineSettings.separation == SplineSettings.Separation.PrefabUpSize)
                {
                    // move along in the spline rotation, considering the extents (/2)
                    addDistanceToDirection = splineRotation * prefab.transform.up * GetDistanceToMove(prefab) / 2;
                }
