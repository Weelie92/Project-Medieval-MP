        ...
        protected override void _StartAction(RPGCharacterController controller, SwitchWeaponContext context)
		{
			...
			if (context.type == "instant") {
				...
				if (changeLeft && changeRight) {
					if (toLeftWeapon == Weapon.Unarmed) { weaponController.InstantWeaponSwitch(toRightWeapon); }
					else { weaponController.InstantWeaponSwitch(toLeftWeapon); }
				}
				else if (changeLeft) {

					// Sheath Left.
					if (controller.hasDualWeapons && !changeRight) { weaponController.InstantWeaponSwitch(fromLeftWeapon); }
					else { weaponController.InstantWeaponSwitch(toLeftWeapon); }
				}
				else if (changeRight) {

					// Sheath Right.
					if (controller.hasDualWeapons && !changeLeft) { weaponController.InstantWeaponSwitch(fromRightWeapon); }
					else { weaponController.InstantWeaponSwitch(toRightWeapon); }
				}
			}
