        ...
        public override void OnInspectorGUI()
		{
			...
			if (mode.value.enumValueIndex == ((int)VolumetricFogMode.On))
            {
                ...
                void DrawQualityOption()
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
                        if(qualityLevel.value.intValue == 0)
                            SetLowQualitySettings();
                        if (qualityLevel.value.intValue == 1)
                ...
                {
                    EditorGUILayout.Space();
                    EditorGUILayout.LabelField(new GUIContent("Color Overrides", "You can override the default fog color settings using the properties in this section. The color ramp is combined multiplicatively with the individual color settings. The resulting value is lerped to according to the influence slider."), EditorStyles.boldLabel);
				...
				{
                    EditorGUILayout.Space();
                    EditorGUILayout.LabelField(new GUIContent("Directional Lighting", "You can tint the fog relative to the main light direction. This effect is most prominent when the main light is close to the horizon."), EditorStyles.boldLabel);
                ...
                {
                    EditorGUILayout.Space();
                    EditorGUILayout.LabelField(new GUIContent("Raymarch", "Configure settings that affect the baseline quality of the Volumetric Fog."), EditorStyles.boldLabel);
