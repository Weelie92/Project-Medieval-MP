...
namespace OccaSoftware.Buto
{
    public class ButoRenderFeature : ScriptableRendererFeature
    {
        class RenderFogPass : ScriptableRenderPass
        {
            ...
            private RenderTargetHandle finalFogTarget;
            ...
            private const string upscaleShaderPath = "Shader Graphs/UpscaleButoShader";

            private const string fogUpscaleTexId = "_ButoTexture";
            ...
            public RenderFogPass()
            {
                ...
                finalFogTarget.Init(fogUpscaleTexId);
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
				...
				if (IsTaaEnabled())
				{
                    ...
                    upscaleInput = taaTarget;
                }
                
                cmd.SetGlobalTexture(upscaleInputTexId, upscaleInput.Identifier());
                ...
                // Writes to final fog
                Blit(cmd, source, finalFogTarget.Identifier(), upscaleMaterial);
                ...
                void SetMaterialParameters()
				{
                    ...
                    void SetFogMaterialData()
					{
                        ...
                        fogMaterial.SetColorArray(Params.SimpleColor.Id, new Color[] { volumetricFog.litColor.value, volumetricFog.shadowedColor.value, volumetricFog.emitColor.value });
                    ...
                    void SetAdditionalLightData()
                    {
                        ...
                        Vector4[] directions = new Vector4[ButoCommon._MAXLIGHTCOUNT];
                        Vector4[] angles = new Vector4[ButoCommon._MAXLIGHTCOUNT];
                        ...
                        {
                            positions[i] = ButoLight.Lights[i].LightPosition;
                            intensities[i] = ButoLight.Lights[i].LightIntensity;
                            colors[i] = ButoLight.Lights[i].LightColor;
                            directions[i] = ButoLight.Lights[i].LightDirection;
                            angles[i] = ButoLight.Lights[i].LightAngles;
                        }
                        ...
                        if (lightCount > 0)
                        {
                            ...
                            fogMaterial.SetVectorArray(Params.LightDirectionButo.Id, directions);
                    ...
                    void SetVolumeData()
                    {
                        ...
                        Vector4[] sizes = new Vector4[ButoCommon._MAXVOLUMECOUNT];
                        float[] intensities = new float[ButoCommon._MAXVOLUMECOUNT];
                        float[] blendModes = new float[ButoCommon._MAXVOLUMECOUNT];
                        float[] blendDistance = new float[ButoCommon._MAXVOLUMECOUNT];
                        float[] shapes = new float[ButoCommon._MAXVOLUMECOUNT];
                        ...
                        {
                            positions[i] = FogDensityMask.FogVolumes[i].transform.position;
                            sizes[i] = FogDensityMask.FogVolumes[i].Size;
                            shapes[i] = (int)FogDensityMask.FogVolumes[i].Shape;
                            intensities[i] = FogDensityMask.FogVolumes[i].DensityMultiplier;
                            blendModes[i] = (int)FogDensityMask.FogVolumes[i].Mode;
                        ...
                        if (volumeCount > 0)
                        {
                            fogMaterial.SetVectorArray(Params.VolumePosition.Id, positions);
                            fogMaterial.SetVectorArray(Params.VolumeSize.Id, sizes);
                            fogMaterial.SetFloatArray(Params.VolumeIntensity.Id, intensities);
                            fogMaterial.SetFloatArray(Params.VolumeBlendMode.Id, blendModes);
                            fogMaterial.SetFloatArray(Params.VolumeBlendDistance.Id, blendDistance);
                            fogMaterial.SetFloatArray(Params.VolumeShape.Id, shapes);
                        }
                    }
